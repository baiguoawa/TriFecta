...
final class SquirrelPanel: NSPanel {
  ...
  func setGroupMode(_ on: Bool) {
    // 只在状态真正变化时才动画，避免每打一个字都闪
    guard view.showsGroups != on else {
      if !on, view.selectedGroup != nil { view.selectedGroup = nil; view.needsDisplay = true }
      return
    }
    view.showsGroups = on
    if !on { view.selectedGroup = nil }
    animateColorChange()
    view.needsDisplay = true
  }
  func setSelectedGroup(_ g: Int?) {
    guard view.selectedGroup != g else { return }
    view.selectedGroup = g
    animateColorChange()
    view.needsDisplay = true
  }
  // 干净的非线性交叉溶解（可靠）
  private func animateColorChange() {
    ...
    trans.type = .fade
    view.layer?.add(trans, forKey: "groupColorChange")
  }
