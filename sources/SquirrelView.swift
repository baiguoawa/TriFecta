...
final class SquirrelView: NSView {
  ...
  // 三色分组：候选按 index/3 分成 3 组，每组一个半透明“彩色玻璃”色（红黄绿，高对比）
  ...
  // 聚焦动画状态跟踪（仅在真实状态变化时触发缩放）
  private var prevShowsGroups = false
  private var prevSelectedGroup: Int?
  // 选中组内各候选的独立胶囊路径（按组内位置索引）
  var candidateItemPaths: [CGMutablePath?] = [nil, nil, nil]
  // 按下前的高亮蓝色（高对比、半透明彩色玻璃）
  var highlightColor = NSColor(calibratedRed: 0.16, green: 0.45, blue: 0.95, alpha: 0.68)
  ...
  override func draw(_ dirtyRect: NSRect) {
    ...
    var candidateGroupPaths: [CGMutablePath?] = [nil, nil, nil]
    var highlightedPath: CGMutablePath?
    var highlightedPreeditPath: CGMutablePath?
    let theme = currentTheme
    let groupSize = max(1, Int((Double(candidateRanges.count) / 3.0).rounded(.up)))
    ...
    if showsGroups {
      // 分组模式：未选中组时每组一条胶囊；选中某组后，该组内候选按位置红/黄/绿，其它组变暗
      candidateGroupPaths = [nil, nil, nil]
        ...
        if let selG = selectedGroup, selG == g {
          ...
          for i in startIdx..<endIdx {
            if let cp = drawPath(highlightedRange: candidateRanges[i], backgroundRect: backgroundRect, preeditRect: preeditRect,
              ...
              candidateItemPaths[i - startIdx]?.addPath(cp)
            }
          }
        } else {
          // 未选中组：整组一条胶囊
      ...
      // 非分组模式：仅高亮当前候选（蓝色胶囊），其余不留底色
      for i in 0..<candidateRanges.count {
        let candidate = candidateRanges[i]
        if i == hilightedIndex, candidate.length > 0, theme.highlightedBackColor != nil {
    ...
    let panelRadius = backgroundRect.height / 2   // 两端半圆（直径=高度）
    backgroundPath = drawSmoothLines(rectVertex(of: backgroundRect), straightCorner: Set(), alpha: 0.3 * panelRadius, beta: 1.4 * panelRadius)
    ...
    if theme.mutualExclusive {
      ...
      for path in candidateGroupPaths.compactMap({ $0 }) {
        backPath?.addPath(path)
      }
    }
    ...
    if !showsGroups, let path = highlightedPath {
